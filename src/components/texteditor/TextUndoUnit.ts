import type { IUndoUnit } from "./IUndoUnit";
...
export default class TextUndoUnit implements IUndoUnit {
  ...
  public redo() {
    ...
    if (this._redoSelection === null) {
      const position: TextPosition = this._textModel.toScreenPosition(
        this._undoRange.end
      );
  ...
  public merge(undoUnit: IUndoUnit): boolean {
    if (undoUnit instanceof SelectionUndoUnit) {
      const selectionUndoUnit: SelectionUndoUnit =
        undoUnit as SelectionUndoUnit;
