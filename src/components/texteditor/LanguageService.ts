...
export default class LanguageService {
  ...
  private window_setTimeout() {
    ...
    this._textEditor.invalidateRange(
      new TextRange(startPosition, new TextPosition(this._line, this._column))
    );
  ...
  private moveRange(oldPosition: TextPosition, newPosition: TextPosition) {
    if (oldPosition.compareTo(newPosition) < 0) {
      ...
      if (this._syntaxTable[oldPosition.line]) {
        const syntax = this._syntaxTable[oldPosition.line].splice(
          index,
        ...
        if (size > 0) {
          ...
          const tail = this._syntaxTable.splice(
            oldPosition.line + 1,
      ...
      if (this._syntaxTable[oldPosition.line]) {
        ...
        while (
          ...
          newPosition.column > this._syntaxTable[newPosition.line][index].start
        ) {
          index++;
        }
  ...
  private clearRange(startPosition: TextPosition, endPosition: TextPosition) {
    if (startPosition.line === endPosition.line) {
      ...
      if (this._syntaxTable[startPosition.line]) {
        for (
          let i = this._syntaxTable[startPosition.line].length - 1;
          i >= 0;
          i--
        ) {
          if (
      ...
      if (this._syntaxTable[endPosition.line]) {
        for (
          let i = this._syntaxTable[endPosition.line].length - 1;
          i >= 0;
          i--
        ) {
          if (
    ...
    if (
      ...
      if (column >= current.start) {
        ...
        this._syntaxTable[this._line].splice(
          this._index,
          0,
          new LanguageStyle(style, state, column)
        );
        this._index++;
      }
    } else {
