...
import TextUndoUnit from "./TextUndoUnit";
...
import type { TextChangeHandler } from "./TextChangeHandler";
import type { SelectionChangeHandler } from "./SelectionChangeHandler";
...
export default class TextEditor {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this._textBuffer_textChanging = (e: TextChangeEvent) => {
      this.textBuffer_textChanging(e);
    };
    this._textBuffer_textChanged = (e: TextChangeEvent) => {
      this.textBuffer_textChanged(e);
    };
    this._textModel_selectionChanged = (e: SelectionChangeEvent) => {
      this.textModel_selectionChanged(e);
    };
  ...
  public removeEventListener(type: string, callback: (e) => void) {
    switch (type) {
      ...
      case "textchanged":
        ...
        break;
      case "selectionchanged":
        this._selectionChangedHandlers = this._selectionChangedHandlers.filter(
          (item) => item !== callback
        );
        break;
    }
  ...
  public selectWord(line: number, column: number) {
    const textPosition: TextPosition = this._textModel.toBufferPosition(
      new TextPosition(line, column)
    );
    const text: string = this._textBuffer.getLine(textPosition.line);
    const startColumn: number = this._textModel.findWordBreak(
      text,
      textPosition.column + 1,
      -1
    );
    const endColumn: number = this._textModel.findWordBreak(
      text,
      textPosition.column,
      1
    );
    const textRange: TextRange = new TextRange(
      new TextPosition(textPosition.line, startColumn),
  ...
  public copy() {
    const textRange: TextRange = this._textModel.toBufferRange(
  ...
  public getTextPosition(point: Point): TextPosition {
    ...
    const column: number = Math.floor(
      (point.x - paddingLeft) / this.fontSize.width
    );
    const line: number = Math.floor(
      (point.y - paddingTop) / this.fontSize.height
    );
  ...
  public invalidate() {
    this.invalidateRectangle(
      new Rectangle(0, 0, this._canvas.width, this._canvas.height)
    );
  }
  ...
  public invalidateRange(textRange: TextRange) {
    ...
    if (textRange.start.line === textRange.end.line) {
      ...
      const width: number =
        (range.end.column - range.start.column) * fontSize.width;
      this.invalidateRectangle(new Rectangle(x, y, width, fontSize.height));
    } else {
      const height: number =
        (range.end.line - range.start.line + 1) * fontSize.height;
  ...
  private updateFont() {
    const fontSize: number =
      parseFloat(this._theme.fontSize) * this.devicePixelRatio;
  ...
  private textBuffer_textChanging(e: TextChangeEvent) {
    ...
    const textRange: TextRange = this._textModel.toScreenRange(
      e.oldRange.normalize()
    );
