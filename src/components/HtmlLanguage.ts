...
export default class HtmlLanguage implements ILanguage {
  ...
  private readAttributeValue(): string {
    ...
    const attributeName: string =
      this._tokenStack[this._tokenStack.length - 1].name.toUpperCase();
    const elementName: string =
      this._tokenStack[this._tokenStack.length - 2].name.toUpperCase();
    if (
      (attributeName === "TYPE" && elementName === "SCRIPT") ||
      (attributeName === "TYPE" && elementName === "STYLE")
    ) {
  ...
  private readLanguage(): any {
    ...
    if (c === "<" || c === "]") {
      ...
      if (
        this._token.contentData === 0 &&
        this.testIgnoreCase(this._token.closeTag)
      ) {
        this.pop();
        return this.read();
      }
    ...
    result.state =
      result.state !== null
        ? this._token.mimeType + ":" + this._token.closeTag + ":" + result.state
        : null;
    return result;
  }
